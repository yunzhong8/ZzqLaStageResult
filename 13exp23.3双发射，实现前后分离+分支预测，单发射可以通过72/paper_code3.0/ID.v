...
/****************************************input decode(输入解码)***************************************/
    ...
    assign {branch_i,btb_hit_i,pht_state_i,pre_pc_i,
            excep_en_i,excep_type_i,pc_i,inst_i} = if_to_ibus;
...
/****************************************output code(输出解码)***************************************/
    ...
    assign  now_is_branch_inst_o = is_branch_inst & now_ctl_valid;
...
//ID阶段使用
  ...
  //  ################计算WB(regs_write)输出################//         
    ...
    //################跳转计算################//         
      ...
      //本处应该修改的
      assign jmp_addr_o    = jmp_flag ? (jmp_base_addr + jmp_offs_addr) :pc_i +32'd4;
     ...
     //分支预测
     wire [1:0]branch_pra_pre = {jmp_flag,branch_i};
     parameter PA0PE0 =2'b00;
     parameter PA0PE1 =2'b01;
     parameter PA1PE0 =2'b10;
     parameter PA1PE1 =2'b11;
     ...
     assign is_branch_inst = inst_i[30];
      ...
      assign {branch_flush,pht_we,pht_wdata_o,
                           btb_we,btb_wdata_o} = is_branch_inst ? jmp_flag ?  (branch_i ? (pre_pc_i ==jmp_addr_o  ? {1'b0,~pht_state_i[0],pht_state_i+2'd1,1'b0,32'd0} : {1'b1,~pht_state_i[0],pht_state_i+2'd1,1'b1,jmp_addr_o} ) :{1'b1,1'b1,pht_state_i+2'd1,1'b1,jmp_addr_o}) :
                                                                        ...
                                                                        (branch_i ?{1'b1,1'b1,pht_state_i-2'd1,1'b0,32'd0} : {1'b0,pht_state_i[0],pht_state_i-2'd1,1'b0,32'd0}) :  {1'b0,1'b0,pht_state_i-2'd1,1'b0,32'd0};
     
      assign branch_flush_o =  branch_flush & now_ctl_valid;
      assign pht_we_o = pht_we & now_ctl_valid;
      assign btb_we_o = btb_we;
      assign {btb_wtag_o,btb_waddr_o} = pc_i[31:3];
      assign btb_wvalid_o = now_ctl_valid;
    ...
    assign want_done_en_o    = (!(spMemReq || div_en||spTlbRe||spTlbSe||spTlbIe||spTlbWe||spTlbFe) && id_valid_i ) ||  ~id_valid_i    ; 
