...
//总线宽度
    ...
    //preif
         ...
         `define PreifToIfBusLen  `EnLen + `ExcepBusLen +`PcLen    + `ExcepBusLen +`PcLen
     ...
     //if
       //线级
            `define LineIfToIdBusLen   `EnLen+  `EnLen+ 2 +`PcLen+  `ExcepBusLen+  `PcInstBusLen//1+1+2+32+
    ...
    //Id阶段
        //流水条级的数据传输
            ...
            `define LineIdToPrBusLen `BtbWbusLen + `PhtWbusLen
            `define LineIdToPrBusWidth `LineIdToPrBusLen-1 :0
        ...
        //模块级的数据传输
            ...
            `define IdToIfBusLen `EnLen//cache_we,tlb_flush,branch_flush
    ...
    //EXE阶段
        ...
        //线级数据传输
            `define LineExToMemBusLen  `EnLen+`PcLen+   `EnLen+`EnLen+  `EnLen+`EnLen+`EnLen+`EnLen+`EnLen +5+2+ `EnLen+  `EnLen+`LineRegsReadObusLen+   `ExcepBusLen+  `LlbitWriteBusLen+  `EnLen+`LineCsrWriteBusLen    +`spMemRegsWdataSrcLen+`spMemMemDataSrcLen+`EnLen+`MemAddrLen   +`EnLen+`LineRegsWriteBusLen  +`PcInstBusLen
    ...
    //MEM阶段       
        //线级
            ...
            `define LineMemToWbBusLen `EnLen+`PcLen+ `EnLen+`EnLen+  `EnLen+`EnLen+`EnLen+`EnLen+`EnLen+5+`EnLen+ `EnLen +`TlbIndexLen+`EnLen+`EnLen+`LineRegsReadObusLen+`ExcepBusLen+`MemAddrLen+`LlbitWriteBusLen+`EnLen+`LineCsrWriteBusLen+`EnLen+`LineRegsWriteBusLen +`PcInstBusLen
