...
//指令信号宽度
    ...
    //例外信号
        `define spExcepLen 4
        `define spExcepWidth `spExcepLen-1:0
        
        `define spExcepTypeLen 2
        `define spExcepTypeWidth `spExcepTypeLen-1 :0
 //信号宽度
  
    `define SignLen          `IdSignLen+`ExSignLen+`MemSignLen+`WbSignLen+`BJSignLen+`spExcepLen//信号实际使用长度16+4+8+8+4
