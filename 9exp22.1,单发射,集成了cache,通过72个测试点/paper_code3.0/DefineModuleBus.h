...
//总线宽度
    ...
    //EXE阶段
        ...
        //线级数据传输
        `define LineExToMemBusLen  `EnLen+  `EnLen+`EnLen+`EnLen+`EnLen+`EnLen +5+2+ `EnLen+  `EnLen+`LineRegsReadObusLen+   `ExcepBusLen+  `LlbitWriteBusLen+  `EnLen+`LineCsrWriteBusLen    +`spMemRegsWdataSrcLen+`spMemMemDataSrcLen+`EnLen+`MemAddrLen   +`EnLen+`LineRegsWriteBusLen  +`PcInstBusLen
    ...
    //MEM阶段
        ...
        //线级
        `define LineMemToWbBusLen `EnLen+  `EnLen+`EnLen+`EnLen+`EnLen+`EnLen+5+`EnLen+ `EnLen +`TlbIndexLen+`EnLen+`EnLen+`LineRegsReadObusLen+`ExcepBusLen+`MemAddrLen+`LlbitWriteBusLen+`EnLen+`LineCsrWriteBusLen+`EnLen+`LineRegsWriteBusLen +`PcInstBusLen
